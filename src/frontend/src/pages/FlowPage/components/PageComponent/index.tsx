...
import { useReactFlowPerformance } from "@/hooks/use-reactflow-performance";
import { useMemoizedNodeFiltering, useMemoizedEdgeCalculations } from "@/utils/memoizedReactflowUtils";
import { useCanvasVirtualization } from "@/hooks/use-canvas-virtualization";
import { useDragPerformance } from "@/hooks/use-drag-performance";
...
export default function Page({
  ...
  const { handleNodesChange, handleEdgesChange, isValidConnection: performantIsValidConnection } = useReactFlowPerformance();
  const memoizedNodes = useMemoizedNodeFiltering(nodes, null);
  const memoizedEdges = useMemoizedEdgeCalculations(edges, nodes);
  
  // Canvas virtualization for large flows
  const { visibleNodes, visibleEdges, stats: virtualizationStats } = useCanvasVirtualization(
    memoizedNodes, 
    memoizedEdges, 
    nodes.length > 20 // Enable virtualization for flows with >20 nodes
  );
  
  // Drag performance optimization
  const { 
    onNodeDragStart: optimizedDragStart, 
    onNodeDrag: optimizedDrag, 
    onNodeDragStop: optimizedDragStop,
  ...
  // Critical Performance Optimization: Debounce expensive helper line calculations
  const debouncedHelperLines = useMemo(() => {
    if (!helperLineEnabled) return () => {};
    
    return _.debounce((node: any, allNodes: AllNodeType[]) => {
      const currentHelperLines = getHelperLines(node, allNodes);
  ...
  const onNodeDrag: OnNodeDrag = useCallback(
    (_, node) => {
      if (helperLineEnabled && nodes.length < 100) { // Only enable for smaller graphs
        debouncedHelperLines(node as any, nodes);
      }
    },
    [helperLineEnabled, nodes, debouncedHelperLines],
  );
  ...
  const onNodesChangeWithHelperLines = useCallback(
    (changes: NodeChange<AllNodeType>[]) => {
      ...
      handleNodesChange(modifiedChanges, onNodesChange);
    },
  ...
  // Memoize fitViewOptions to prevent object recreation on every render
      ...
      {showCanvas ? (
        <div id="react-flow-id" className="h-full w-full bg-canvas">
          ...
          <ReactFlow<AllNodeType, EdgeType>
            ...
            onEdgesChange={(changes) => handleEdgesChange(changes, onEdgesChange)}
